...
- The server currently responds with "Hello, World!" to all requests
- HTTP request parsing will be implemented in Stage 3
- Static file serving will be implemented in Stage 4
- The server uses non-blocking I/O with `poll()` for efficient event handling
- Connection timeouts are set to 60 seconds by default
